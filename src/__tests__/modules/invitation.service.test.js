...
describe('raising an invitation', () => {
  it('records the tenancy, the invitee and who invited them', async () => {
    const res = await service.createInvitation({
      tenantId: TENANT, phone: '98765 43210', roleIds: ['role-1'], invitedBy: ADMIN,
    });
    const insert = sqlOf(/INSERT INTO tenant_invitations/)[0];
    // id, tenant, phone, isAdmin, fullName, branch, invitedBy, expiresAt
    expect(insert.params.slice(0, 7))
      .toEqual(['mock-uuid', TENANT, '+919876543210', 0, null, null, ADMIN]);
  ...
  // it is claimed. Otherwise a new joiner shows up as a bare number that
  // somebody then has to identify — which is worse than the email era, where
  // the address usually carried a name inside it.
  it('carries the staff details so the person arrives named', async () => {
    await service.createInvitation({
      tenantId: TENANT, phone: '+919876500001', invitedBy: ADMIN,
      profile: { fullName: 'Priya R', branchDetailId: 'branch-1' },
    });
    expect(sqlOf(/INSERT INTO tenant_invitations/)[0].params.slice(4, 6))
      .toEqual(['Priya R', 'branch-1']);
  });

  // Every spelling of one number must reach the same row, or the claim at login
  // misses and the person is auto-provisioned a tenancy of their own instead.
  it.each(['98765 43210', '09876543210', '+91-98765-43210', '0091 9876543210'])(
    'normalises %p before storing it',
    async (typed) => {
      await service.createInvitation({ tenantId: TENANT, phone: typed, invitedBy: ADMIN });
      expect(sqlOf(/INSERT INTO tenant_invitations/).pop().params[2]).toBe('+919876543210');
    },
  );

  it('refuses a number it cannot normalise', async () => {
    await expect(service.createInvitation({
      tenantId: TENANT, phone: 'not-a-number', invitedBy: ADMIN,
    })).rejects.toThrow(/valid mobile number/i);
  });

  it('carries an expiry so a forgotten invitation does not stay live forever', async () => {
    await service.createInvitation({ tenantId: TENANT, phone: '+919000000001', invitedBy: ADMIN });
    const expiresAt = sqlOf(/INSERT INTO tenant_invitations/)[0].params[7];
  ...
  it('can invite a co-admin', async () => {
    await service.createInvitation({ tenantId: TENANT, phone: '+919000000001', isAdmin: true, invitedBy: ADMIN });
    expect(sqlOf(/INSERT INTO tenant_invitations/)[0].params[3]).toBe(1);
  });

  it('attaches the roles the invitee will receive', async () => {
    await service.createInvitation({
      tenantId: TENANT, phone: '+919000000001', roleIds: ['role-1', 'role-2'], invitedBy: ADMIN,
    });
  ...
  it('refuses to invite an existing member', async () => {
    ...
    await expect(service.createInvitation({
      tenantId: TENANT, phone: '+919000000001', invitedBy: ADMIN,
    })).rejects.toThrow(/already in this tenancy/i);
  });

  it('refuses a second live invitation for the same number', async () => {
  ...
  it('refuses roles that do not belong to the inviting tenancy', async () => {
    await expect(service.createInvitation({
      tenantId: TENANT, phone: '+919000000001', roleIds: ['role-from-elsewhere'], invitedBy: ADMIN,
...
describe('claiming at login', () => {
  const invite = (over = {}) => ({
    id: 'inv-1', tenant_id: TENANT, phone: '+919000000001', is_admin: 0, ...over,
  });
  ...
  it('stamps the staff details onto the new membership', async () => {
    state.claimable = [invite({ full_name: 'Priya R', branch_detail_id: 'branch-1' })];

    await service.acceptPendingTx(mockConn, '+919000000001');

    const profile = sqlOf(/UPDATE user_tenants SET full_name/)[0];
    // No phone among these: the membership's number is user_phone, in the
    // WHERE clause, not a profile field that could drift from the identity.
    expect(profile.params).toEqual(['Priya R', 'branch-1', '+919000000001', TENANT]);
  });
  ...
  it('treats an existing membership as success rather than failing the login', async () => {
    ...
    await expect(service.acceptPendingTx(mockConn, '+919000000001')).resolves.toHaveLength(1);
    expect(sqlOf(/SET status = 'ACCEPTED'/)).toHaveLength(1);
  });
  ...
  // Case-insensitivity was the email era's concern. The equivalent here is
  // that any spelling of the number reaches the same pending invitation.
  it.each(['9000000001', '09000000001', '+91 90000 00001'])(
    'claims the invitation when the number is typed as %p',
    async (typed) => {
      await service.acceptPendingTx(mockConn, typed);
      expect(executed[0].params[0]).toBe('+919000000001');
    },
  );

  it('claims nothing when the number cannot be normalised', async () => {
    const claimed = await service.acceptPendingTx(mockConn, 'nonsense');
    expect(claimed).toEqual([]);
