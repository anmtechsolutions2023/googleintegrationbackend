...
// admin picks one; an INVITATION is raised BY a tenancy, for an email that may
...
// "is there anything to claim for this email?" at the right moment.
...
/**
 * Invite an email into ONE tenancy.
 *
 ...
 *        than as a bare email somebody has to identify afterwards.
 * @param {string} p.invitedBy
 * @returns {Promise<Object>} The created invitation.
 */
const createInvitation = ({ tenantId, email, roleIds = [], isAdmin = false, profile = {}, invitedBy }) =>
  withTransaction(async (conn) => {
    ...
    try {
      await conn.execute(QUERIES.INVITATIONS.INSERT, [
        ...
        profile.fullName ?? null, profile.phone ?? null, profile.branchDetailId ?? null,
        invitedBy, expiresAt,
      ]);
    } catch (e) {
      // uq_invite_live is a PARTIAL unique index (see the schema): one live
      // invitation per tenant+email, unlimited closed history beside it.
    ...
    return { id, tenantId, email: invitee, isAdmin: !!isAdmin, roleIds, profile, expiresAt };
  });
...
/**
 * Claim every live invitation for a verified email. Called by the login path.
 *
 ...
 * @param {string} email - The Google-verified address.
 * @returns {Promise<Array<{tenantId:string, roleCount:number}>>} What was claimed.
 */
const acceptPendingTx = async (conn, email) => {
  const invitee = normalizeEmail(email);
  ...
  for (const invite of invitations) {
    ...
    try {
      await conn.execute(QUERIES.ADMIN_USERS.INSERT_USER_TENANT_FLAGS, [
        uuidv4(), invitee, invite.tenant_id, invite.is_admin ? 1 : 0, 0,
      ]);
    } catch (e) {
    ...
    for (const roleId of roleIds) {
      try {
        await conn.execute(QUERIES.USER_ROLES.INSERT, [
          uuidv4(), invitee, invite.tenant_id, roleId, invite.email,
        ]);
      } catch (e) {
        if (e.code !== 'ER_DUP_ENTRY') throw e;
      }
    ...
    if (invite.full_name || invite.phone || invite.branch_detail_id) {
      await conn.execute(QUERIES.ADMIN_USERS.UPDATE_PROFILE, [
        invite.full_name ?? null, invite.phone ?? null, invite.branch_detail_id ?? null,
    ...
    if (roleIds.length === 0) {
      ...
      logger.warn('Invitation accepted with no surviving roles', {
        email: invitee, tenantId: invite.tenant_id, invitationId: invite.id,
      });
    }
    claimed.push({ tenantId: invite.tenant_id, roleCount: roleIds.length });
  }

  logger.info('Invitations claimed at login', { email: invitee, count: claimed.length });
  return claimed;
};
