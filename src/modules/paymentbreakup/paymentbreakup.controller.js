// src/modules/paymentbreakup/paymentbreakup.controller.js
const service = require('./paymentbreakup.service');
...
const {
  ...
  paginationSchema,
  uuidParamSchema,
} = require('./paymentbreakup.schemas');
...
const getAll = asyncHandler(async (req, res) => {
  ...
  paginatedResponse(
    ...
    result.data,
    result.pagination,
    'Payment breakups retrieved successfully'
  );
});

const getById = asyncHandler(async (req, res) => {
  const data = await service.getById(req.params.id, req.user.tid);
  successResponse(res, data, 'Payment breakup retrieved successfully');
});

const create = asyncHandler(async (req, res) => {
  const newData = await service.create(req.body, req.user.tid, req.user.email);
  createdResponse(res, newData, 'Payment breakup created successfully');
});
...
const update = asyncHandler(async (req, res) => {
  ...
  successResponse(res, updated, 'Payment breakup updated successfully');
});

const deleteById = asyncHandler(async (req, res) => {
  await service.delete(req.params.id, req.user.tid);
  noContentResponse(res, 'Payment breakup deleted successfully');
});
