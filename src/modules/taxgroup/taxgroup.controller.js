// src/modules/taxgroup/taxgroup.controller.js
const service = require('./taxgroup.service');
...
const getAll = asyncHandler(async (req, res) => {
  ...
  paginatedResponse(
    ...
    result.data,
    result.pagination,
    'Tax groups retrieved successfully'
  );
});

const getById = asyncHandler(async (req, res) => {
  const data = await service.getById(req.params.id, req.user.tid);
  successResponse(res, data, 'Tax group retrieved successfully');
});

const create = asyncHandler(async (req, res) => {
  const newData = await service.create(req.body, req.user.tid, req.user.email);
  createdResponse(res, newData, 'Tax group created successfully');
});
...
const update = asyncHandler(async (req, res) => {
  ...
  successResponse(res, updated, 'Tax group updated successfully');
});

const deleteById = asyncHandler(async (req, res) => {
  await service.delete(req.params.id, req.user.tid);
  noContentResponse(res, 'Tax group deleted successfully');
});
