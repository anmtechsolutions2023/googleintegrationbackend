// src/modules/mapprovider/mapprovider.controller.js
const service = require('./mapprovider.service');
...
const getAll = asyncHandler(async (req, res) => {
  ...
  paginatedResponse(
    ...
    result.data,
    result.pagination,
    'Map providers retrieved successfully'
  );
});

const getById = asyncHandler(async (req, res) => {
  const data = await service.getById(req.params.id, req.user.tid);
  successResponse(res, data, 'Map provider retrieved successfully');
});

const create = asyncHandler(async (req, res) => {
  const newData = await service.create(req.body, req.user.tid, req.user.email);
  createdResponse(res, newData, 'Map provider created successfully');
});
...
const update = asyncHandler(async (req, res) => {
  ...
  successResponse(res, updated, 'Map provider updated successfully');
});

const deleteById = asyncHandler(async (req, res) => {
  await service.delete(req.params.id, req.user.tid);
  noContentResponse(res, 'Map provider deleted successfully');
});
