...
const swaggerSpec = {
  ...
  components: {
    ...
    schemas: {
      ...
      Category: {
        ...
        properties: { ...auditFields,
          ...
          TagIds: {
            ...
            description: 'Menu tags applied to the SECTION. Every dish filed under it inherits them, so one assignment here covers a whole category instead of one per dish. Omit to leave the existing links alone; send [] to detach all.',
          },
          ItemCount: {
            type: 'integer', readOnly: true,
            description: 'How many catalogue items are filed under this category. Present so the form can say what tagging the section will reach — it is a bulk edit that touches every dish here.',
          },
          SortOrder: { type: 'integer' },
        },
      },
      ...
      AppConfig: {
        ...
        properties: {
          ...
          timezone: {
            ...
            description: 'IANA zone the trading day runs on. Category schedules are written in the outlet\'s local time and evaluated against this, NOT against the server clock — which is UTC in production. Reported as the effective value, so a never-written setting shows the default actually in use. Platform-wide: app_settings has no tenant_id.',
          },
        },
      },
      AppConfigUpdate: {
        ...
        properties: {
          ...
          timezone: {
            type: 'string', example: 'Asia/Kolkata',
            description: 'IANA zone name. Refused at validation if this Node build does not know it — a plausible typo like Asia/Kolkatta would otherwise move every schedule onto the fallback clock silently.',
          },
        },
      },
      ...
      PosItemNutrition: {
        ...
        description: 'Every figure is optional, and so is the block itself. Send null (or an empty string, which a cleared panel posts) to remove the nutrition row; omit the key entirely to leave it as it is.',
        properties: {
      ...
      PosItemMetaCreate: {
        ...
        properties: {
          ...
          ServesCount: {"type":"integer","minimum":0,"maximum":255,"nullable":true,"description":"How many people the dish serves. Optional. An empty string is accepted and stored as null, because that is what a cleared form control posts."},
          PortionSize: {"type":"string","maxLength":50,"nullable":true,"description":"The measure, e.g. 350 ml. Optional. Answers a different question from ServesCount."},
          MeatTypeId: {"type":"string","format":"uuid","nullable":true,"description":"Orthogonal to FoodTypeId: a dish is Non-Veg AND Chicken. Optional. An empty string is accepted and stored as null, because that is what a cleared form control posts."},
          PrepTimeMinutes: {"type":"integer","minimum":0,"nullable":true,"description":"This dish\u2019s own prep time, in minutes. Optional. An order KPT is derived from its slowest line. An empty string is accepted and stored as null, because that is what a cleared form control posts."},
      ...
      PosMenuTagRef: {
        type: 'object',
        description: 'A menu tag as it is returned alongside a dish — id, name and which vocabulary it belongs to.',
        properties: {
          id: { type: 'string', format: 'uuid' },
          name: { type: 'string', example: 'Chinese' },
          type: { type: 'string', enum: ['CATEGORY', 'BEVERAGE', 'CUISINE'], example: 'CUISINE' },
        },
      },
      PosItemMeta: {
        type: 'object',
        properties: { ...auditFields,
          ...
          ItemName: {
            type: 'string', nullable: true, readOnly: true,
            description: 'The dish name, from the catalogue row this menu entry extends. Present so a till never has to resolve names one request per dish.',
          },
          OwnTags: {
            type: 'array', readOnly: true,
            items: { $ref: '#/components/schemas/PosMenuTagRef' },
            description: 'Tags set on THIS dish (pos_item_meta_tag), resolved to names. Write them through TagIds.',
          },
          CategoryTags: {
            type: 'array', readOnly: true,
            items: { $ref: '#/components/schemas/PosMenuTagRef' },
            description: 'Tags inherited from the dish\'s category (pos_category_tag), resolved to names. Kept separate from OwnTags rather than merged: a client shows an inherited tag differently from one set on the dish. The effective set is the union of the two.',
          },
          CategoryAvailableNow: {
            type: 'boolean', readOnly: true,
            description: 'Whether the dish\'s category is inside one of its scheduled windows right now. TRUE when the category has no schedule at all — no rules means always available. Evaluated on the APP server\'s clock, not the database\'s.',
          },
          CategoryOpensAt: {
            type: 'string', nullable: true, readOnly: true, example: '18:00:00',
            description: 'When the category next opens, for a client to label a greyed-out dish. The next window still to come today, else the earliest it ever opens. Null while it is available.',
          },
          Channels: {"type":"object"},
          Prices: {"type":"object"},
          Variants: {"type":"object"},
          ...
          lines: {
            ...
            items: {
              ...
              properties: {
                ...
                ref: { type: 'string', maxLength: 3672, example: '7dd63848-ae87-4707-87e6-290abb341665|2336bc80-7530-4332-afc1-39982911b7ed', description: 'Client correlation key, echoed back untouched. Built by the till as the menu row id, then each variant id, then each add-on id, joined with |. When the same dish with the same options carries a different kitchen note, the till appends a short suffix (|n2) to keep it a separate line. The cap is derived from the line limits (1 + 20 variants + 50 add-ons + 1 split segment, each up to 50 characters): a flat 100 was exceeded by one dish with two add-ons, which failed the whole request.' },
              },
            },
          },
          discount: {
      ...
      PosOrderCreate: {
        ...
        properties: {
          ...
          Status: {"type":"string"},
          Items: {"type":"array", "items": {"type":"object"}, "description": "Order lines. Each line may carry `note` — the kitchen note for that dish (for example \"Less spicy, No onion\"), up to 140 characters at the till; longer is refused with 400 naming the line. The note is copied to the kitchen ticket and to the invoice line. A portal line's `notes` is read as the same thing."},
      ...
      PosKot: {
        ...
        properties: { ...auditFields,
          ...
          FiredAt: {"type":"string","format":"date-time"},
          BranchDetailId: {"type":"string","format":"uuid"},
          CookingInstructions: {"type":"string","nullable":true,"description":"The whole-order note as it was when the ticket was sent. A snapshot: editing the order later does not change it."},
          NoCutlery: {"type":"boolean"},
          Active: {"type":"boolean"},
        },
      },
      PosBillSettle: {
        ...
        properties: {
          ...
          lines: {
            ...
            items: {
              ...
              properties: {
                ref: { type: 'string', maxLength: 3672, example: 'order-1#0', description: 'The line key the till already uses for a hand-typed discount. Same key and same cap as the pricing quote, so the two cannot disagree.' },
      ...
      ReportProducts: {
        ...
        properties: {
          ...
          products: { type: 'array', items: { type: 'object', properties: {
            ...
            OptionsAmount: { type: 'number', description: 'What options (variants) added on top of the dish price: per-unit surcharge × quantity, before discount. Part of GrossAmount.' },
            AddonsAmount: { type: 'number', description: 'What add-ons added on top of the dish price, on the same basis. Part of GrossAmount.' },
          } } },
        },
      },
      ReportOptions: {
        type: 'object',
        description: 'Which options and add-ons sell, on which dishes, and how often each is taken. Read from the snapshot on each invoice line, so renaming or repricing a choice does not rewrite history. Same filters and document scope as the product report.\n\n**TakeRate** = plates that took the choice ÷ plates of the dishes that offer it (mapped on the menu today, or sold with it in the period). Null when nothing offered it.',
        properties: {
          range: reportRange,
          totals: { type: 'object', properties: {
            OptionsAmount: { type: 'number' }, AddonsAmount: { type: 'number' },
            GrossAmount: { type: 'number' }, Plates: { type: 'number' },
            ShareOfRevenue: { type: 'number', nullable: true, description: 'Percent of GrossAmount that came from options and add-ons.' },
          } },
          variants: { type: 'array', items: { type: 'object', properties: {
            VariantId: { type: 'string', nullable: true }, Name: { type: 'string' }, Price: { type: 'number' },
            Plates: { type: 'number' }, OfferedPlates: { type: 'number' }, TakeRate: { type: 'number', nullable: true },
            Revenue: { type: 'number' },
            Dishes: { type: 'array', items: { type: 'object', properties: { ItemId: { type: 'string' }, ItemName: { type: 'string' }, Plates: { type: 'number' } } } },
          } } },
          addonGroups: { type: 'array', items: { type: 'object', properties: {
            GroupId: { type: 'string', nullable: true }, GroupName: { type: 'string' }, MaxSelection: { type: 'number', nullable: true },
            Plates: { type: 'number', description: 'Plates that took at least one add-on from the group.' },
            OfferedPlates: { type: 'number' }, TakeRate: { type: 'number', nullable: true }, Revenue: { type: 'number' },
            Addons: { type: 'array', items: { type: 'object', properties: {
              AddonId: { type: 'string', nullable: true }, Name: { type: 'string' }, Price: { type: 'number' },
              Plates: { type: 'number' }, TakeRate: { type: 'number', nullable: true }, Revenue: { type: 'number' },
            } } },
          } } },
          products: { type: 'object', description: 'Keyed by ItemId, for dishes that sold with or offer a choice: the same breakdown for that one dish, with PlatesWithoutOption.', additionalProperties: { type: 'object' } },
        },
      },
      ReportPending: {
        type: 'object',
      ...
      PosSettings: {
        ...
        properties: {
          ...
          'kitchen.note_presets': {
            description: 'Quick-pick kitchen notes Billing offers, in display order. Read back as JSON text; '
              + 'written as an array of up to 20 notes, each 1–40 characters, no duplicates ignoring case. '
              + 'An empty list means the cashier types every note.',
            oneOf: [
              { type: 'string', example: '["Less spicy","Extra spicy","Less salt","Less oil","No onion","No garlic","Jain"]' },
              { type: 'array', maxItems: 20, items: { type: 'string', minLength: 1, maxLength: 40 } },
            ],
          },
        },
      },
  ...
  paths: {
    ...
    '/api/ledger/reports/options': reportPath('LedgerReports', 'Options and add-ons: what sells and how often it is taken', 'ReportOptions',
      'Take rates per option, per add-on group and per add-on, across the menu and per dish.'),
    ...
    '/api/pos/item-meta/bulk': {
      patch: {
        tags: ['PosItemMetas'],
        summary: 'Change many menu items at once',
        description: 'One change applied to every listed menu row, all or nothing. If any id no longer exists the whole request is refused with 404 and nothing is written.\n\n'
          + '**Active: false** turns dishes off: Billing greys them and a new order naming one is refused ("Not on sale: …"), even when the dish\'s section is inside its trading hours. Rounds already placed are not affected.\n\n'
          + 'Link fields (ChannelIds, VariantIds, AddonGroupIds, TagIds) take a mode: **add** keeps each dish\'s existing links, **remove** drops only the listed ones, **replace** sets exactly these. Tags a dish inherits from its category are not touched.\n\n'
          + 'Item, Branch and Price cannot be bulk-changed. Writes one audit entry per dish (one summary entry above 50).',
        security,
        requestBody: { required: true, content: { 'application/json': { schema: {
          type: 'object', required: ['ids', 'changes'],
          properties: {
            ids: { type: 'array', minItems: 1, maxItems: 500, uniqueItems: true, items: { type: 'string', format: 'uuid' } },
            changes: { type: 'object', minProperties: 1, properties: {
              Active: { type: 'boolean' },
              FoodTypeId: { type: 'string', format: 'uuid' },
              MeatTypeId: { type: 'string', format: 'uuid', nullable: true, description: 'null or "" clears it.' },
              PrepTimeMinutes: { type: 'integer', minimum: 0, nullable: true },
              ServesCount: { type: 'integer', minimum: 0, maximum: 255, nullable: true },
              ChannelIds: { type: 'object', required: ['mode', 'ids'], properties: { mode: { type: 'string', enum: ['add', 'remove', 'replace'] }, ids: { type: 'array', maxItems: 200, items: { type: 'string', format: 'uuid' } } } }, VariantIds: { type: 'object', required: ['mode', 'ids'], properties: { mode: { type: 'string', enum: ['add', 'remove', 'replace'] }, ids: { type: 'array', maxItems: 200, items: { type: 'string', format: 'uuid' } } } }, AddonGroupIds: { type: 'object', required: ['mode', 'ids'], properties: { mode: { type: 'string', enum: ['add', 'remove', 'replace'] }, ids: { type: 'array', maxItems: 200, items: { type: 'string', format: 'uuid' } } } }, TagIds: { type: 'object', required: ['mode', 'ids'], properties: { mode: { type: 'string', enum: ['add', 'remove', 'replace'] }, ids: { type: 'array', maxItems: 200, items: { type: 'string', format: 'uuid' } } } },
            } },
          },
          example: { ids: ['7dd63848-ae87-4707-87e6-290abb341665'], changes: { Active: false } },
        } } } },
        responses: {
          200: { description: 'Updated. data: { updated, items: [{ Id, ItemName, Active }] }' },
          404: { description: 'One or more ids no longer exist; nothing was written.' },
          ...responses.validation, ...responses.unauthorized, ...responses.forbidden,
        },
      },
    },
